...
export default async function SendMail(req, res) {
  ...
  // const googleUrl = 'https://www.google.com/recaptcha/api/siteverify?secret=' + process.env.RECAPTCHA_SECRET_KEY + '&response=' + req.body.captcha;
  const googleUrl = `https://www.google.com/recaptcha/api/siteverify?secret=${process.env.RECAPTCHA_SECRET_KEY}&response=${req.body.captcha}`;
  ...
  if (IsFormValid && validateCaptcha.data.success === true) {
    ...
    transporter.verify((error) => {
      ...
      return transporter.sendMail(mailData, (err) => {
        if (err) {
  ...
  return formSchema.validate(req.body, { abortEarly: false }).catch((error) => {
    // collecte de toutes les erreurs
    const errors = error.inner.reduce((acc) => ({
      ...acc,
