...
export const getServerSideProps = ({ res }) => {
  ...
  const staticPagesBase = fs
    // fs.readdirSync est la méthode de répertoire de lecture synchone intégrée à Node.js.
    // Permet d'obtenir la liste des fichiers du répertoire "pages"
    .readdirSync(
      {
        development: 'pages',
  ...
  const staticPagesProjets = fs
    // fs.readdirSync est la méthode de répertoire de lecture synchore intégré à Node.js.
    // Permet d'obtenir la liste des fichiers du répertoire "MesProjets"
    .readdirSync(
      {
    ...
    // on mappe la liste pour retourner une url à chaque element de la liste
    .map((staticPagePath) => `${baseUrl}/MesProjets/${staticPagePath}`)
    .map((path) => (
      path.replace('.html', '')
    ));
