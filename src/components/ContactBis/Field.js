...
const FieldContact = () => {
  ...
  const handleSubmitForm = async (evt) => {
    ...
    // abortEarly empéche l'abandon de la vérification après la première erreur
    const IsFormValid = await formSchema.isValid(data, { abortEarly: false });
    ...
    if (!IsFormValid || !isVerified) {
      setMessageAlert(messagesAlert.error);
      // si la validation des champs du formulaire a échoué
      // on récupère les erreurs des champs incorrects
      formSchema.validate(data, { abortEarly: false }).catch((error) => {
      // on collecte les messages erreurs de chaque champs
        const errorsMessage = error.inner.reduce((acc, err) => ({
          ...acc,
